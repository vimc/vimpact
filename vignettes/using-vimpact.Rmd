...
The simplest possible example is a static model with constant routine immunisation and a one-off campaign. Burden estimates from a theoretical country are found in 2 csv titled `example_baseline_burden.csv` and `example_focal_burden.csv`. These include estimates of burden in years 1900 - 2100 and display the format used by the functions.

```{r}
example_baseline <- read.csv(system.file("extdata/example_baseline_burden.csv", package = "vimpact"))
example_focal <- read.csv(system.file("extdata/example_focal_burden.csv", package = "vimpact"))
...
```{r}
calendar_year <- vimpact::impact_by_calendar_year(example_baseline, example_focal)
str(calendar_year)
```
...
```{r}
birth_year <- vimpact::impact_by_birth_year(example_baseline, example_focal)
str(birth_year)
```
...
activity_type <- vimpact::impact_by_year_of_vaccination_activity_type(example_baseline, example_focal, example_fvps, 2000:2030)
str(activity_type)
```
...
```{r}
cohort_type <- vimpact::impact_by_year_of_vaccination_birth_cohort(example_baseline, example_focal, example_fvps, 2000:2030)
str(cohort_type)
```
